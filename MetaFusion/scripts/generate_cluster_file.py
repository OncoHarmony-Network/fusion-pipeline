...
def output_clustered_fusions(fusion_list, cluster_type):
        ...
        #closest exons
        exon1 = ",".join(oset([f.closest_exon1 for f in fusion_list]))
        exon2 = ",".join(oset([f.closest_exon2 for f in fusion_list]))
        #categories
        ...
        print "\t".join(map(str, [cluster_type, ",".join(list(oset(gene1_list))), ",".join(list(oset(gene2_list))), max_split_cnt, max_span_cnt, ",".join(list(oset(sample_type_list))), ",".join(list(oset(disease_list))), ",".join(list(oset(tool_list))), ",".join(list(oset(category_list))), gene1_on_bndry, gene1_close_to_bndry, gene2_on_bndry, gene2_close_to_bndry, cancer_db_hits, ",".join(list(oset(sample_list))), ",".join(list(oset(chr1_list))), "|".join(list(oset(breakpoint_1_list))), ",".join(list(oset(chr2_list))), "|".join(list(oset(breakpoint_2_list))), exon1, exon2 ,",".join(list(oset(fusion_IDs)))]))
